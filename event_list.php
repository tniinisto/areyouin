<?php
	...
	$sql = "select v.Events_eventID, l.name as location,l.position as pos, e.startTime, e.endTime, p.playerid, p.name, p.photourl, v.areyouin, m.teamID, m.teamName, a.teamAdmin from events e, eventtype t, location l, players p,  eventplayer v, team m, playerteam a where t.eventTypeID = e.EventType_eventTypeID and l.locationID = e.Location_locationID and p.playerID = v.Players_playerID and v.Events_eventID = e.eventID and a.Players_playerID = p.playerID and a.Team_teamID = m.teamID and m.teamID = '" . $teamid  . "' order by e.startTime asc, v.Events_eventID asc";
	...
	while($row = mysql_fetch_array($result))
	{
		...
		echo "<table border='0' class=\"atable2\">";
			echo "<tr>";
				...
				echo "<td class=\"col2\"><img width=\"40\" height=\"40\" src=\"images/" . $row['photourl'] . "\"></td>";
				echo "<td class=\"col3\">" . $row['name'] . "</td>";
				if($row['areyouin'] == 0)
					echo "<td class=\"col4\">OUT</td>";
				else
					echo "<td class=\"col4\">IN</td>";
			echo "</tr>";
		echo "</table>";		
	}	
